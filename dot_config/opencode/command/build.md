...
Invoke the incremental-implementation skill using the `skill` tool alongside the test-driven-development skill using the `skill` tool.
...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke the planning-and-task-breakdown skill using the `skill` tool to generate one.
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → follow the debugging-and-error-recovery skill using the `skill` tool
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → follow the doubt-driven-development skill using the `skill` tool and get explicit sign-off before continuing
...
If any step fails, follow the debugging-and-error-recovery skill using the `skill` tool.
